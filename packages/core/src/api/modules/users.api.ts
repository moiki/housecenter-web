...
export const usersApi = {
  list: (page = 1, pageSize = DEFAULT_PAGE_SIZE) =>
    getApiClient().get<PagedResult<UserResponse>>(BASE, { params: { page, pageSize } }).then(r => r.data),

  getById: (id: string) =>
    getApiClient().get<UserResponse>(`${BASE}/${id}`).then(r => r.data),

  update: (id: string, data: UpdateUserRequest) =>
    getApiClient().put<UserResponse>(`${BASE}/${id}`, data).then(r => r.data),

  deactivate: (id: string) =>
    getApiClient().delete<void>(`${BASE}/${id}`).then(r => r.data),

  assignRoles: (id: string, data: AssignRolesRequest) =>
    getApiClient().put<UserResponse>(`${BASE}/${id}/roles`, data).then(r => r.data),
